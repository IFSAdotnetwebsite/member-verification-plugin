...
class Ifsa_Member_Verification_Public {
	 ...
	 *
	 * @return funciton it returns the enqueued styles
	 * @since 1.0.0
	 *
	 */
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/ifsa-member-verification-public.js', array( 'jquery' ), $this->version, false );
		wp_enqueue_script( 'ifsa-renewal-form_js', plugin_dir_url( __FILE__ ) . 'js/ifsa-renewal-form.js', array( 'jquery' ), $this->version, false );
		
		
		include_once ABSPATH . 'wp-admin/includes/plugin.php';
		if ( is_user_logged_in() ) {
			//show buddypress user profile page
			global $current_user;
			$profileURL = get_bloginfo( 'url' ) . '/members/' . $current_user->user_login . '/profile/';
		}
		
		wp_localize_script(
			'ifsa-renewal-form_js', 'ifsa_script_vars', array(
				'	'  => $profileURL,
...
<aside class="bp-feedback bp-messages ifsa-response-reject" style="visibility: hidden;">
			...
			<div id="myModal_remove" class="modal">
				...
				<div class="modal-content">
					...
					<div class="ul_reason">
						...
						<input type="radio" id="membership_expire" name="reason" value="Your LC Membership has expired.">
						...
						<label for="You have never been an LC Member">The applicant user has never been an LC member</label></br>
				...
				foreach ( $res as $memberlistresult ) {
					...
					<tr>
						...
						<?php if ( $memberlistresult->member_status == 0 && $memberlistresult->reason == '' ) { ?>
							<td data-title="Action">
								<div class="<?php echo esc_attr_e( 'cls-action-' . $memberlistresult->id . '' ); ?>"><a href="javascript:void(0)"
	...
	public function ifsa_multistep_form() {
		...
		if ( ! ( is_user_logged_in() ) ) {
			include_once plugin_dir_path( __FILE__ ) . 'partials/ifsa-member-verification-public-display.php';
		} else {
...
/**
				...
				if ( $today <= $end && $today >= $beforedate) {
					...
					<form action="" method="post">
						...
						<div class="ifsa_memebr_expiry_wrap">
							...
							<div id="ifsa_renewal_form-sucess-profile" class="ifsa_renewal_form-sucess-profile" style="display:none;text-align:center;padding-top:30px !important;">
				...
				<?php } else { ?>
					<div class="ifsa_memebr_expiry_wrap">
						...
						<div style="float:right;">
							...
							<span disabled="disabled" style="color:#333333;background:#c5c5c5 !important;" id="ifsa-verify_member-btn">Verify Membership</span>
						</div>
					</div>
				<?php }
	...
	public function ifsa_register_user_front_end() {
		...
		$new_user_last_name  = strtolower( $_POST['new_user_lname'] );
		$new_user_first_name = $_POST['new_user_fname'];
		
		if (filter_var($new_user_email, FILTER_VALIDATE_EMAIL) == false) {
			$notice_key = 'Please add valid email address';
				echo $notice_key;
				wp_die();
		}
		$user_data = array(
			...
			'user_nicename' => $new_user_first_name,
		...
		if ( ! is_wp_error( $user_id ) ) {
			...
			if ( isset( $_COOKIE['stepperform'] ) && ! empty( $_COOKIE['stepperform'] ) ) {
				$stepperform = json_decode( stripslashes( strval( sanitize_text_field( $_COOKIE['stepperform'] ) ) ) );
			}
			if (  $stepperform->utm_source == 'invite' ) {
				$source = 'Invite';
			} else if (isset( $_GET['utm_source'] ) && !empty($_GET['utm_source'])) {
				$source = 'Invite';
			}else {
				$source = 'Website';
			}
			...
			xprofile_set_field_data( 'Gender', $user_id, $_POST['ddlgender'] );
			xprofile_set_field_data( 'Nationality', $user_id, $_POST['ddlnationality'] );
			...
			$stepperForm = array();
			if ( isset( $_COOKIE['stepperform'] ) && ! empty( $_COOKIE['stepperform'] ) ) {
				$stepperform = json_decode( stripslashes( sanitize_text_field( $_COOKIE['stepperform'] ) ) );
				update_user_meta( $user_id, 'is_student', $stepperform->isStudent );
				xprofile_set_field_data( 'Student status', $user_id, $stepperform->isStudent );
				
				update_user_meta( $user_id, 'graduateday', $stepperform->graduateday );
				xprofile_set_field_data( 'Graduation day', $user_id, $stepperform->graduateday );
				
				update_user_meta( $user_id, 'is_ifsa_member', $stepperform->isifsaMember );
				
				update_user_meta( $user_id, 'university_name', $stepperform->universityname );
				xprofile_set_field_data( 'University Name', $user_id, $stepperform->universityname );
				
				update_user_meta( $user_id, 'ifsa_country', $stepperform->country );
				xprofile_set_field_data( 'University Country', $user_id, $stepperform->country );
				
				update_user_meta( $user_id, 'university_level', $stepperform->universityLevel );
				xprofile_set_field_data( 'University Level', $user_id, $stepperform->universityLevel );
				update_user_meta( $user_id, 'course_topic', $stepperform->courseTopic );
				xprofile_set_field_data( 'Course Topic', $user_id, $stepperform->courseTopic );
				unset( $_COOKIE['stepperform'] );
			}
	...
	public function ifsa_lcadmin_banner() {
			...
			This is an LC admin account use it only for admin purposes not for personal stuff. Click <a id="logout-submit" href="<?php echo wp_logout_url();?>">here</a> to logout and login with your personal account
			</p></div>
			<?php }
